...
class DNHeadv2(BaseModule):
    def __init__(self,
                 ...
                 num_points = 72,
                 seg_feat = 256,
                 seg_decoder_feat = sum([128,256,512]),
                 sample_points=36,
        ...
        self.reg_length = nn.Linear(self.seg_feat, 1)  # 1 length
        self.reg_layers = nn.Linear(self.seg_feat, self.n_offsets)  # n offsets
        nn.init.constant_(self.fc_reg.layers[-1].weight.data, 0)
        nn.init.constant_(self.fc_reg.layers[-1].bias.data, 0)
        nn.init.constant_(self.reg_layers.weight.data, 0)
        nn.init.constant_(self.reg_layers.bias.data, 0)
    ...
    def forward(self, hidden_states: Tensor,
        ...
        # todo : unclder operation
        #check unclear offset
        predicition[..., 6:] = (
    ...
    def loss(self,
        ...
        for idx,predictions_layer in enumerate(prediction): 
            #each layer is defined as last layer

            predictions_layer = [pred for pred in predictions_layer] 
            is_last_list = [True  for pred in predictions_layer]
            
            ota_matched_col_inds,ota_matched_row_inds,match_col_inds,match_row_inds,is_empty_list = multi_apply(self.gengeral_label_assign,predictions_layer,targets_list)
