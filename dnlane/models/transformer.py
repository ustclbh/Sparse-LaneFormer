...
class DABDetrTransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def _init_layers(self):
        ...
        others."""
        self.self_attn = ConditionalAttention(**self.self_attn_cfg)  #normal multi-head attn
...
#        intermediate_reference_points = []
        ...
        if self.return_intermediate:
            return [
                torch.stack(intermediate),   # (num_layer, Nq, bs, c)
                torch.stack(intermediate_reference_points), # (num_layer,Nq, bs, 3)
                torch.stack(intermediate_offest_points) # (num_layer, Nq, bs, 72)
            ]
        else:
            return [
                output.unsqueeze(0),
